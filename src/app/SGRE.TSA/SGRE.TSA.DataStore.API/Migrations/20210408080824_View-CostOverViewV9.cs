...
{
    public partial class ViewCostOverViewV9 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var createAndDropViewsql = @"DROP VIEW IF EXISTS cost_overview;
                                CREATE OR REPLACE VIEW cost_overview AS
                                    ...
                                    SELECT scenario_id, 1 as ""position"", SUM(nomination) as nomination, SUM(offer) as offer, SUM(signature) as signature,
                                    case when nomination <= 0 THEN 1 ELSE 0 END as nomination_check,
                                    case when offer <= 0 THEN 1 ELSE 0 END as offer_check,
                                    case when signature <= 0 THEN 1 ELSE 0 END as signature_check
                                     FROM cost_feed_back cfb
                                    JOIN costs_tower_ex_works ctew 
                                    on ctew.cost_feedback_id = cfb.id
                                    JOIN costs_tower_ex_works_line_item ctewli
                                    ON ctewli.costs_tower_ex_works_id = ctew.id
                                    GROUP BY scenario_id, nomination_check, offer_check, signature_check
                                    UNION ALL
                                    SELECT scenario_id, 2 as ""position"", SUM(ctllt.nomination) as nomination, SUM(ctllt.offer) as offer, SUM(ctllt.signature) as signature,
                                    case when nomination <= 0 THEN 1 ELSE 0 END as nomination_check,
                                    case when offer <= 0 THEN 1 ELSE 0 END as offer_check,
                                    case when signature <= 0 THEN 1 ELSE 0 END as signature_check
                                    FROM cost_feed_back cfb
                                    JOIN costs_tower_logistics ctl 
                                    on ctl.cost_feedback_id = cfb.id
                                    JOIN costs_tower_logistics_supplier ctls 
                                    on ctls.costs_tower_logistics_id = ctl.id
                                    JOIN costs_tower_logistics_line_item ctllt
                                    ON ctllt.costs_tower_logistics_supplier_id = ctls.id
                                    GROUP BY scenario_id, nomination_check, offer_check, signature_check
                                    UNION ALL
                                    SELECT scenario_id, 3 as ""position"", SUM(ctc.cost_nomination) as nomination, SUM(ctc.cost_offer) as offer, SUM(ctc.cost_signature) as signature,
                                    case when cost_nomination <= 0 THEN 1 ELSE 0 END as nomination_check,
                                    case when cost_nomination <= 0 THEN 1 ELSE 0 END as offer_check,
                                    case when cost_nomination <= 0 THEN 1 ELSE 0 END as signature_check
                                    FROM cost_feed_back cfb
                                    JOIN costs_tower_construction ctc 
                                    on ctc.cost_feedback_id = cfb.id
                                    GROUP BY scenario_id, nomination_check, offer_check, signature_check
                                    UNION ALL
                                    SELECT scenario_id, 4 as ""position"", SUM(ctclt.nomination) as nomination, SUM(ctclt.offer) as offer, SUM(ctclt.signature) as signature,
                                    case when nomination <= 0 THEN 1 ELSE 0 END as nomination_check,
                                    case when offer <= 0 THEN 1 ELSE 0 END as offer_check,
                                    case when signature <= 0 THEN 1 ELSE 0 END as signature_check
                                    FROM cost_feed_back cfb
                                    JOIN costs_tower_customs ctcs 
                                    on ctcs.cost_feedback_id = cfb.id
                                    JOIN costs_tower_customs_supplier ctcss
                                    on ctcss.costs_tower_customs_id = ctcs.id
                                    JOIN costs_tower_customs_line_item ctclt
                                    ...
                                    UNION ALL
                                    SELECT scenario_id, 6 as ""position"", CASE WHEN s.scenario_type = 0 THEN 0 ELSE SUM(ct.tower_component_certificate_windfarm_cluster_nomination/t.turbine_quantity) END as nomination, 
                                    CASE WHEN s.scenario_type = 0 THEN 0 ELSE SUM(ct.tower_component_certificate_windfarm_cluster_offer/t.turbine_quantity) END as offer,
                                     CASE WHEN s.scenario_type = 0 THEN 0 ELSE SUM(ct.tower_component_certificate_windfarm_cluster_signature/t.turbine_quantity) END as signature,
                                    1 as nomination_check,
                                    1 as offer_check,
                                    1 as signature_check
                                     FROM cost_feed_back cfb
                                    JOIN costs_technology ct 
                                    ...
                                    UNION ALL
                                    SELECT s.id as scenario_id, 8 as ""position"", rc.cost as nomination, rc.cost as offer, rc.cost as signature, 
                                    0 as nomination_check,
                                    0 as offer_check,
                                    0 as signature_check  FROM rna_costs rc 
                                    ...
                                    UNION ALL SELECT t2.scenario_id, 9 as ""position"", 
                                    case WHEN SUM(nomination_check) > 2 THEN 0  ELSE round(sum(nomination),3) END  as nomination, 
                                    case WHEN SUM(offer_check) > 2 THEN 0 ELSE round(sum(offer),3) END as offer, 
                                    case WHEN SUM(signature_check) > 2 THEN 0 ELSE round(sum(signature),3) END as signature
                                    FROM total t2 GROUP BY t2.scenario_id )

                                                SELECT rt.scenario_id, rt.""position"",
                                              
                                                nomination,
                                                round(rt.nomination * t.turbine_quantity,3) as nomination_windfarm, 
                                                offer,
                                                round(rt.offer * t.turbine_quantity,3) as offer_windfarm, 
                                                signature,
                                                round(rt.signature * t.turbine_quantity,3) as signature_windfarm, s.scenario_type
